<template>
  <div class="dropdown">
    <input type="text" :value="value" @input="updateValue" class="border-[1px] border-[#163331] w-full bg-transparent bg-opacity-50 rounded-md">
    <ul v-if="isLoading" class="z-10 bg-white bg-opacity-50 backdrop-blur-2xl rounded-md overflow-hidden w-full"> 
    ...
    <ul v-if="isDropdownOpen" class="z-10 bg-white bg-opacity-50 backdrop-blur-2xl rounded-md overflow-hidden w-full"> 
      <li class="px-4 py-2 border-[#163331] hover:text-white hover:bg-[#163331] px-4 py-2" v-for="item in dataArray" :key="item" @click="selectOption(item)">{{item.name}}, {{ item.cityName }} ({{ item.countryName }})</li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    debouncedSearch: _debounce(function(searchValue) {
      ...
      axios.post(`${this.apiDomain}/api/get-airports-list`, payload, {
        headers: {
